...
def load_items() -> Dict[str, Item]:
    ...
    with ITEMS_CSV.open() as f:
        ...
        for row in reader:
            items[row["item"]] = Item(
                name=row["item"], weight=float(row["weight"]), cost=float(row["cost"])
            )
    return items


def export_items(items: Dict[str, Item], path: Optional[Path] = None) -> None:
    ...
    with path.open("w", newline="") as f:
        ...
        for item in items.values():
            writer.writerow(
                {"item": item.name, "weight": item.weight, "cost": item.cost}
            )
